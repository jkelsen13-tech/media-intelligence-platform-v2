...
function harness({rows,manifest}, options = {}) {
  ...
  const destination = {
    rpc:async (action,input) => {
      ...
      if (options.breakAt === action) throw Error('synthetic interruption with private content omitted')
      if (action==='enqueue') {
        ...
        if (!job) {job={id:`00000000-0000-0000-0000-${String(jobs.size+1).padStart(12,'0')}`,run_id:input.run_id,records:input.records,pageHash:hash(input.records.map(r=>({id:r.source_id,sha256:r.payload_sha256}))),completed:false};jobs.set(input.run_id,job)}
...
test('exact native fields, numeric precision, Unicode, nulls, vector text and version readback',async()=>{
  ...
  const r=await executeParentCustody({manifest:f.manifest,...h,authorization:{mode:'synthetic'}})
...
test('duplicate delivery resumes completed exact jobs without claim',async()=>{
  const f=fixture(),h=harness(f)
  assert.equal((await executeParentCustody({manifest:f.manifest,...h,authorization:{mode:'synthetic'}})).state,'readback_verified')
  h.calls.length=0
  assert.equal((await executeParentCustody({manifest:f.manifest,...h,authorization:{mode:'synthetic'}})).state,'readback_verified')
...
test('lost acknowledgement leaves incomplete then exact retry reads committed state',async()=>{
  ...
  let r=await executeParentCustody({manifest:f.manifest,...h,authorization:{mode:'synthetic'}})
  ...
  const second=await executeParentCustody({manifest:f.manifest,...resume,authorization:{mode:'synthetic'}})
...
test('native staging RPC preserves exact source-qualified versions and private/public boundaries',async t=>{
  ...
  const jobs=(await db.query('select run_id,source_project_ref,source_table,state,page_size from legacy_graph_staging.import_jobs order by run_id')).rows
  ...
  assert.equal(Number((await db.query('select count(*)::int n from legacy_graph_staging.record_conflicts')).rows[0].n),1)
  assert.equal(Number((await db.query('select count(*)::int n from legacy_graph_staging.payload_versions')).rows[0].n),5)
  result=await executeParentCustody({manifest:f.manifest,source:h.source,destination:native,
    authorization:{mode:'synthetic'}})
