...
import { membershipInputFingerprint as fingerprint } from '../supabase/runtime-snapshots/source-comparison-run-v12/membershipFingerprint.js'
import { scoreEventMembership, regressionMixedTopicMembershipFixture } from '../supabase/runtime-snapshots/source-comparison-run-v12/lib.js'
...
test('the complete runtime changes only fingerprint binding and preserves scoring, projection, authorization and queue logic',()=>{
  assert.equal(read('v12','lib.js'),read('v10','lib.js'))
  assert.equal(read('v12','loadedLanguageLexicon.json'),read('v10','loadedLanguageLexicon.json'))
  let next=read('v12','index.ts')
