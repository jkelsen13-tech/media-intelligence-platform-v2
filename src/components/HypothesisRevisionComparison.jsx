...
export default function HypothesisRevisionComparison({view,revisionId}){
 ...
 return<section className="piw-card piw-revision-comparison" aria-label="Compare saved hypothesis revisions">
  ...
  <p>Saved result: {diff.effect.replaceAll('_',' ')}. These differences do not calculate a new likelihood or confidence.</p>
  ...
  <details onToggle={event=>{if(event.target===event.currentTarget)setOpen(event.currentTarget.open)}}>
   ...
   {open?<div>
    ...
    <p>Completion timestamps are not proof of historical commit visibility.</p>
    ...
    <p>This comparison does not acknowledge review, resolve pending work, approve permissions or authorize publication.</p>
   </div>:null}
  </details>
 </section>
}
