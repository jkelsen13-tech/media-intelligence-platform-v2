...
// The temporary administrator can SET this package owner but does not inherit
// its forced-RLS table rights. Enter owner context only for exact CAS ledger DML.
async function withCasOwner(admin, work) {
...
export async function bootstrapAuthenticatedOperation(admin, config) {
  ...
  try {
    ...
    const casScopePresent = await withCasOwner(admin, async () =>
      ...
      || (await admin.query('select 1 from mip_cas.source_permissions where investigation=$1::uuid',[investigation])).rowCount)
    if (casScopePresent) throw Error('cnc_cas_scope_already_present')
    ...
    await withCasOwner(admin, async () => {
...
export async function cleanupAuthenticatedOperation(admin, operationId) {
  ...
  try {
    ...
    const unrelatedCasAuthority = await withCasOwner(admin, async () =>
      (await admin.query('select 1 from mip_cas.principals where login=$1 and user_id<>$2::uuid',[expected.cas,manifest.userId])).rowCount
      ...
      || (await admin.query('select 1 from mip_cas.source_permissions where investigation=$1::uuid and not(source_version=any($2::text[]))',[manifest.investigation,manifest.sourceVersions])).rowCount)
    if (unrelatedCasAuthority) throw Error('cnc_unrelated_cas_authority')
