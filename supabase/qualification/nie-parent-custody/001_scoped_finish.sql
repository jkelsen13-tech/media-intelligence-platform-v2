...
begin
  ...
  expected_fields := case job.source_table
  ...
  loop
    ...
    if rec->>'source_project_ref' is distinct from job.source_project_ref
       ...
       or rec->>'object_family' is distinct from
         case job.source_table when 'events' then 'source_comparison_event' else 'article' end
