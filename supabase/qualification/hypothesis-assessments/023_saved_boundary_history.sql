...
grant usage on schema mip_temporal to mip_boundary_history_gateway,mip_boundary_proof_issuer;
...
create table mip_temporal.boundary_history_permits(
 ...
 expires_at timestamptz not null,payload jsonb not null,consumed boolean not null default false,
 check(cardinality(revision_ids)<=512),check(octet_length(payload::text)<=1048576)
);
...
revoke all on mip_temporal.boundary_history_permits from public,anon,authenticated,service_role,mip_boundary_history_gateway,mip_boundary_proof_issuer;
...
begin
 h:=mip_hypothesis.read_bound_history(p_user,p_investigation);
 -- Intersect within the definer boundary. Unselected history never reaches the gateway.
...
declare v mip_temporal.source_versions;c mip_temporal.stream_captures;k mip_temporal.boundary_capture_types;
 h mip_temporal.stream_heads;p mip_temporal.boundary_history_permits;actual pg_lsn;ids uuid[];auth_until timestamptz;lease_until timestamptz;
begin
 ...
 p.payload:=mip_temporal.boundary_history_payload((p_claim->>'user_id')::uuid,(p_claim->>'investigation_id')::uuid,ids);
 ...
 lease_until:=least(auth_until,clock_timestamp()+interval '10 seconds',mip_identity.boundary_session_expiry(p_session),mip_hypothesis.boundary_payload_expiry(p.payload));
 ...
 insert into mip_temporal.boundary_history_permits(request_id,backend_pid,transaction_id,source_session,binding_id,
  ...
  user_id,investigation_id,revision_ids,prefix_digest,expires_at,payload)
 values((p_claim->>'request_id')::uuid,(p_claim->>'backend_pid')::integer,(p_claim->>'transaction_id')::xid8,p_session,v.id,
  ...
  encode(sha256(convert_to(array_to_string(ids,','),'UTF8')),'hex'),lease_until,p.payload)
 returning * into p;
...
begin
 ...
 if current_payload is distinct from p.payload or p.expires_at<=clock_timestamp() then raise exception 'mip_boundary_snapshot_changed';end if;
 ...
 return jsonb_build_object('schema','mip_boundary_delivery_v1','permit_id',p.id,'request_id',p.request_id,'expires_at',p.expires_at,'remaining_ms',floor(extract(epoch from (p.expires_at-clock_timestamp()))*1000)::integer,'payload',p.payload);
end $$;
...
begin
 ...
 where n.nspname='mip_temporal' and p.proname in('boundary_history_challenge','boundary_history_payload','issue_boundary_history_permit','consume_boundary_history_permit','guard_history_permit') loop
  execute 'alter function '||r.signature||' owner to mip_temporal_advance_owner';
  execute 'revoke all on function '||r.signature||' from public,anon,authenticated,service_role,mip_temporal_recorder,mip_temporal_ack_gateway,mip_comparison_worker_v1,mip_boundary_history_gateway,mip_boundary_proof_issuer';
 end loop;
end $owners$;
