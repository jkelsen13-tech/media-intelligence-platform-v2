...
export function createHypothesisHandler({authenticate,store,sourceProject,allowedOrigins,generationTarget=null,observationEpoch=null}) {
 ...
 return async request=>{
  ...
  try{
   ...
   if(['capture_observation','read_observation','list_observations'].includes(body.action)&&!uuid(observationEpoch))
    return reply(409,{error:{code:'observations_not_configured'}})
   ...
   const data=body.action==='list_observations'?await store.listObservations({...scope,expectedEpoch:observationEpoch}):body.action==='capture_observation'?await store.captureObservation({...scope,requestId:i.request_id,expectedEpoch:observationEpoch}):body.action==='read_observation'?await store.readObservation({...scope,observationId:i.observation_id,expectedEpoch:observationEpoch}):body.action==='review_history'?await store.reviewHistory(scope):body.action==='acknowledge_review'?await store.acknowledgeReview({...scope,requestId:i.request_id,revisionId:i.revision_id,previousReceiptId:i.previous_receipt_id}):body.action==='recover_generation'?await store.recoverGeneration({...scope,workspaceVersionId:i.workspace_version_id,sourceProject,
