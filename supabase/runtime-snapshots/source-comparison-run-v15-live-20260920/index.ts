...
import { verifyRequestedArticleRows } from './articleInputReadback.js'
import { comparisonProjectionConfig } from './projectionConfig.js'
...
import { MEMBERSHIP_SCORE_READBACK_COLUMNS, verifiedMembershipScoreIds } from './membershipScoreReadback.js'
...
async function rebuildMembershipScores(supabase: any, dryRun: boolean, options: any) {
  ...
  const { data: persistedScores, error: persistedError } = await pagedSelect(
    ...
    MEMBERSHIP_SCORE_READBACK_COLUMNS,
    ['event_id', 'id'],
    PAGE_SIZE,
  ...
  let scoreIdByFingerprint: Map<string, string>
  try {
    scoreIdByFingerprint = verifiedMembershipScoreIds(scoreRows, persistedScores ?? [])
  } catch (error) {
    return { error: `membership score readback verification failed: ${(error as Error).message}` }
  }
...
Deno.serve(async (req: Request) => {
  ...
  catch { return json(500, { error: 'invalid comparison projection configuration' }) }
