...
// Per-call features: no cross-request cache, ID key, retained payload or mutation.
// Keep lexical null handling and cosine arithmetic identical to the baseline.
function prepareMembershipArticle(article) {
  return {
    article,
    title: tokenize(titleText(article)),
    lead: tokenize(articleText(article)),
    embedding: parseEmbedding(article?.embedding),
    actor: properNameTokens(articleText(article)),
    topic: topicTokens(article),
    action: actionTokens(article),
  }
}

function preparedSimilarity(a, b) {
  return !a.size || !b.size ? null : similarityFromSets(a, b)
}

function semanticCoherence(a, b) {
  const lexical = maxObserved([
    preparedSimilarity(a.title, b.title),
    preparedSimilarity(a.lead, b.lead),
  ])
  const embedding = a.embedding && b.embedding ? clamp01(cosine(a.embedding, b.embedding)) : null
...
function pairMembershipSignals(a, b) {
  return {
    semantic: semanticCoherence(a, b),
    actor: setOverlap(a.actor, b.actor),
    topic: setOverlap(a.topic, b.topic),
    temporal: temporalCoherence(a.article, b.article),
    action: setOverlap(a.action, b.action),
  }
}
...
function scoreMemberAgainstPeers(prepared, peers, event) {
  const article = prepared.article
  const pairSignals = peers.map((peer) => pairMembershipSignals(prepared, peer))
  const signals = {}
...
export function scoreEventMembership(event, members, releaseGate = {}) {
  ...
  const prepared = articles.map(prepareMembershipArticle)
  const memberScores = prepared.map((article, index) => scoreMemberAgainstPeers(article, prepared.filter((_, peerIndex) => peerIndex !== index), event))
  ...
  const rawThreshold = releaseGate.autoApprovalThreshold
  const explicitThreshold = typeof rawThreshold === 'number' ||
    (typeof rawThreshold === 'string' && rawThreshold.trim() !== '')
  const threshold = explicitThreshold ? Number(rawThreshold) : NaN
