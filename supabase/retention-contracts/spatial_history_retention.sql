-- Private sandbox-qualified spatial history. No live spatial/publication tables are changed.
create table mip_private.spatial_row_versions (
  source_project text not null check (source_project in ('jfnzyvzthzqtczlxhjll')),
  source_relation text not null check (source_relation in ('spatial.assertion_revisions','spatial.assertions','spatial.audience_scopes','spatial.break_glass_audit','spatial.evidence_artifact_registry','spatial.evidence_condition_events','spatial.evidence_snapshots','spatial.geometry_snapshots','spatial.graph_node_authority_snapshots','spatial.place_authority_snapshots','spatial.policy_artifacts','spatial.release_decisions','spatial.review_decisions','spatial.revision_evidence','spatial.revision_lineage')),
  ...
  check (source_observed_at <= retained_at),
  ...
  check (payload ? 'id'),
  check (jsonb_typeof(payload->'id') = 'string'),
  check (source_key = payload->>'id'),
  check (payload->>'id' is not null)
);
alter table mip_private.spatial_row_versions enable row level security;
revoke all on mip_private.spatial_row_versions from public, anon, authenticated, service_role;
grant select, insert on mip_private.spatial_row_versions to service_role;

create function mip_private.reject_spatial_history_mutation() returns trigger
language plpgsql set search_path='' as $$
begin raise exception 'spatial_history_is_immutable' using errcode='55000'; end $$;
revoke all on function mip_private.reject_spatial_history_mutation() from public, anon, authenticated, service_role;
create trigger spatial_history_no_mutation before update or delete on mip_private.spatial_row_versions
for each row execute function mip_private.reject_spatial_history_mutation();
create trigger spatial_history_no_truncate before truncate on mip_private.spatial_row_versions
for each statement execute function mip_private.reject_spatial_history_mutation();

create function mip_private.retain_spatial_rows(
...
comment on table mip_private.spatial_row_versions is
  'Exact sandbox-qualified spatial snapshots retained during backend consolidation; source decisions are not survivor approvals. Append-only; not publication, evidence admission, or proof of historical source states before observation.';
