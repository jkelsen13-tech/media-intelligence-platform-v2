...
const allowedKeys = {
  ...
  run: [['investigation_id', 'version_id'], []],
}
...
export function createEvidenceChecksHandler({ authenticate, rpc, allowedOrigins = ['https://jkelsen13-tech.github.io'] }) {
...
export function createEvidenceChecksTransport({ url, anonKey, serviceKey, fetchImpl = fetch }) {
  ...
  return {
    ...
    async rpc(action, input) {
      const r = await request('/rest/v1/rpc/mip_investigation_evidence_checks_v1', {
