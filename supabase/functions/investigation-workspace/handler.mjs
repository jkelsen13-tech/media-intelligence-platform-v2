...
const allowedKeys = {
  list: [[], ['after', 'limit']],
  read: [['investigation_id'], ['version_id']],
  mark_review: [['investigation_id', 'version_id', 'receipt_id', 'previous_receipt_id'], []],
}
...
function validate(body) {
  ...
  for (const [k, v] of Object.entries(input)) {
    ...
    else if (['after', 'previous_receipt_id'].includes(k) && v === null) continue
...
export function createWorkspaceHandler({ authenticate, rpc, allowedOrigins = ['https://jkelsen13-tech.github.io'] }) {
...
export function createWorkspaceTransport({ url, anonKey, serviceKey, fetchImpl = fetch }) {
  ...
  return {
    ...
    async rpc(action, input) {
      const r = await request('/rest/v1/rpc/mip_investigation_workspace_v1', {
