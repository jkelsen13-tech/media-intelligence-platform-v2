...
declare user_id uuid; inv uuid:=gen_random_uuid(); ver1 uuid:=gen_random_uuid(); ver2 uuid:=gen_random_uuid();
  ...
  ctx jsonb; a jsonb; position text; evidence jsonb; state jsonb; result jsonb; original jsonb; check_original jsonb; check_new jsonb;
begin
  ...
  check_original:=public.mip_investigation_evidence_checks_v1('read',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver1));
  if check_original->>'status'<>'not_run' then raise exception 'read unexpectedly ran checks'; end if;
  check_original:=public.mip_investigation_evidence_checks_v1('run',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver1));
  if check_original->>'status'<>'saved' or check_original->>'publicly_eligible'<>'false' then raise exception 'check run failed'; end if;
  if public.mip_investigation_evidence_checks_v1('run',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver1)) is distinct from check_original then
    raise exception 'check retry changed report'; end if;
  if exists(select 1 from evidence_pipeline.investigation_review_receipts where investigation_id=inv) then raise exception 'checks marked reviewed'; end if;
  ...
  check_new:=public.mip_investigation_evidence_checks_v1('run',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver2));
  if jsonb_array_length(check_new->'report'->'result'->'lineage_candidates')<1 or not exists(
    select 1 from jsonb_array_elements(check_new->'report'->'result'->'challenge_cues') c where c->'reference'->>'excerpt'='corrected') then
    raise exception 'new evidence cues or same-source links missing'; end if;
  if public.mip_investigation_evidence_checks_v1('read',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver1)) is distinct from check_original then
    raise exception 'old check report rewritten'; end if;
  ...
  begin
    perform public.mip_investigation_evidence_checks_v1('run',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver2));
    raise exception 'viewer ran checks'; exception when insufficient_privilege then null;
  end;
  perform public.mip_investigation_evidence_checks_v1('read',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver2));
  begin
  ...
  begin
    perform public.mip_investigation_evidence_checks_v1('read',jsonb_build_object('user_id',user_id,'investigation_id',inv,'version_id',ver2));
    raise exception 'revoked check read allowed'; exception when insufficient_privilege then null;
  end;
  begin
    perform public.mip_investigation_workspace_v1('read',jsonb_build_object('user_id',user_id,'investigation_id',inv));
...
set local role anon;
do $$ begin
  begin perform public.mip_investigation_evidence_checks_v1('read','{}'); raise exception 'browser RPC allowed'; exception when insufficient_privilege then null; end;
  begin perform count(*) from evidence_pipeline.investigation_evidence_check_reports; raise exception 'browser table allowed'; exception when insufficient_privilege then null; end;
end $$;
reset role;
...
reset role;
rollback;
select 'evidence checks: exact saved versions, correction cues, same-source links, idempotence, no automatic review, viewer/revoked/browser denial passed; all fixtures and assignments rolled back' result;
