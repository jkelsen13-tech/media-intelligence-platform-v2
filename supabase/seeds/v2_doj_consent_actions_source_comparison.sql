-- Isolated v2 Source Comparison seed for the DOJ consent-actions manifest.
-- Three separate primary-source events; no independent-source or shared-claim assertion.
...
WHERE rule_version = 'v2-doj-consent-actions-2026-08-18';
...
VALUES
  (
    ...
    (SELECT id FROM public.arc_events WHERE title = 'DOJ announced dismissal steps for Louisville and Minneapolis actions' AND arc_id = (SELECT id FROM public.story_arcs WHERE slug = 'doj-consent-actions-2025')),
    'active', 'v2-doj-consent-actions-2026-08-18'
  ),
  (
    'DOJ announced support for Seattle termination motion',
    DATE '2025-07-23', DATE '2025-07-23', NULL,
    (SELECT id FROM public.story_arcs WHERE slug = 'doj-consent-actions-2025'),
    (SELECT id FROM public.arc_events WHERE title = 'DOJ announced support for Seattle termination motion' AND arc_id = (SELECT id FROM public.story_arcs WHERE slug = 'doj-consent-actions-2025')),
    'active', 'v2-doj-consent-actions-2026-08-18'
  ),
  (
    ...
    (SELECT id FROM public.story_arcs WHERE slug = 'doj-consent-actions-2025'),
    (SELECT id FROM public.arc_events WHERE title = 'Court granted DOJ motion regarding Norfolk decree' AND arc_id = (SELECT id FROM public.story_arcs WHERE slug = 'doj-consent-actions-2025')),
    'active', 'v2-doj-consent-actions-2026-08-18'
  );
...
SELECT
  ...
  END,
  'fact', false, 'active', 'v2-doj-consent-actions-2026-08-18'
FROM public.events e
WHERE e.rule_version = 'v2-doj-consent-actions-2026-08-18';
...
SELECT
  ...
  'manual', 1.0, 'asserts', '[]'::jsonb, 1, true
...
JOIN public.articles a ON a.url = CASE e.canonical_title
  ...
  WHEN 'Court granted DOJ motion regarding Norfolk decree'
    THEN 'https://www.justice.gov/opa/pr/federal-court-grants-justice-departments-motion-terminate-47-year-old-consent-decree'
END
WHERE c.rule_version = 'v2-doj-consent-actions-2026-08-18';
...
SELECT
  (SELECT count(*) FROM public.events WHERE rule_version = 'v2-doj-consent-actions-2026-08-18') AS seeded_events,
  (SELECT count(*) FROM public.claims WHERE rule_version = 'v2-doj-consent-actions-2026-08-18') AS seeded_claims,
  (SELECT count(*) FROM public.event_articles ea JOIN public.events e ON e.id = ea.event_id WHERE e.rule_version = 'v2-doj-consent-actions-2026-08-18') AS seeded_event_articles,
  (SELECT count(*) FROM public.claim_evidence_links cel JOIN public.claims c ON c.id = cel.claim_id WHERE c.rule_version = 'v2-doj-consent-actions-2026-08-18') AS seeded_primary_links
LIMIT 1;
