...
### September 22 native-shell demonstration projection

The follow-up branch `codex/mip-september-22-demo-20260918` keeps the separate Vite entry and the production entry remains unchanged. The demo entry now shares MIP visual tokens and presents a native investigation workspace with a dedicated `#/demo/<topic>/<surface>/<capture>` namespace, local receipt-only search, investigation context, retained sources, provenance, dependency comparison, attribution-safe graph, source-date timeline, and research-collection membership. It does not reuse the canonical `#/event/` identity contract.

The adapter copies an explicit allowlist of receipt fields and rejects non-pending states, invalid hashes/spans, non-HTTPS links, and any supplied canonical/public/admission identity. It forces `publication_allowed=false`, `public_node_id=null`, a source-publication-only timeline basis, and withheld geography. No Supabase, auth, application backend, operator, RPC, review-decision, admission, or publication module is in the demo import graph. The ordinary production build does not import either receipt manifest.

The built artifact contains the pending receipt metadata and therefore must be served only through an owner-approved internal access-controlled preview channel. `noindex,nofollow` reduces accidental indexing but is not an authorization or privacy boundary. This branch does not select, configure, or deploy such a channel.

The committed receipts do not contain retained excerpt text. The UI therefore labels `statement` as a synopsis and shows exact capture/candidate/hash/span identities while explicitly disclosing that the bounded excerpt remains in the private capture. It does not represent the synopsis as the quoted span or claim to recompute the hash in-browser.
