...
A trigger serializes inserts through one transaction advisory lock and caps retained rows at 1,024 globally and 32 per user, including expired rows that have not been deleted. Each metadata row is capped at 16 KiB. Issuance acquires that lock before source and permission locks and first invokes bounded cleanup. Cleanup removes at most 256 expired rows ordered by expiry/ID and skips locked rows. Consumption does not acquire the quota lock. Fresh rows cannot be deleted through the guard. Table truncation remains denied.

Only the distinct NOLOGIN, NOINHERIT, NOBYPASSRLS cleanup role can execute the cleanup entrypoint; it receives no table access, issuer permission or consumer permission. The function is owned by the existing private owner with empty search path and forced RLS. Issuer and consumer remain table-blind. Tests race two final user-quota inserts and require exactly one success, verify metadata has no payload, reject fresh deletion, inspect role access, and invoke cleanup after natural expiry.
