...
| 19-N | Invalid / stale `claim` / `entity` / `source` / `place` ids fall back to the **parent** Investigation Context with disclosure. They do not select a different subject. Spatial `mip_object_id` (`777b3951-…`) as an entity sub-selection is honest parent-fallback, not a fail and not a new subject. Same IC copy is one line. | **PASS** | `tests/deepLinks.test.mjs` — `invalid/stale sub-selection ID falls back to parent IC, not a different subject`. `tests/investigationJoinState.test.mjs` — `invalid selection IDs fail closed to parent IC with consistent disclosure`; `spatial mip_object_id entity is honest parent-fallback; IC copy is one line`. Live World View B: invalid sub-selection → parent IC. CoS 2026-09-03: accepted, not a fail. |
...
| 19-X | No second invented public event. No Port Meridian / evacuation / AQI / shipping / humidity-cloud widgets. No GEV fork / person-CCTV-aircraft-vessel overlays. Cesium ellipsoid rendering is permitted behind the World View renderer adapter seam (keyless, no ion). | **PASS** | `tests/worldViewLaunch.test.mjs` A / H / K and `src has no globe-vendor / 3D-tile strings (Cesium allowed inside adapter seam only)`. Live World View A / H / K. Closeout invents no second subject. |
...
CoS 2026-09-03: A–K PASS accepted. The `777b3951-…` IC disclosure is honest parent-fallback (spatial `mip_object_id` is not a valid entity sub-selection) — not a fail. Do not invent a subject. Same warning on the Investigation Context bar is one line (`visibleJoinDisclosures`).
