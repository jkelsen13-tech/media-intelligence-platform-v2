...
Initial candidate deecaf847d6b3db8fbee33a25f906b891e57d7e2 native run34811084428 passed 81/82 and failed the new direct-grant assertion. Two interactive psql test commands lacked statement terminators, so the denied command was buffered rather than executed. The correction adds those terminators and retains every denial assertion; no SQL grant or policy is changed. The failed run remains failed evidence. The separate native worker integration passed its actual worker table/function denials at that candidate.
