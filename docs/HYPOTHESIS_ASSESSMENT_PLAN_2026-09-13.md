...
## Human reconsideration workflow (007 continuation)

Assigned reviewers can record an explicit reconsideration request against the current hypothesis revision, including when no new source has arrived. Request categories identify a possible contradiction, shared origin, or methodological/reasoning concern; they do not establish the concern as true or authorize a method. Exact current owner identity and request arguments are required on retry. A concurrent completion either includes a request already committed or makes a later request against the replaced revision reject. No pending request is silently lost.

The cause and request receipt commit together in separate immutable tables. Backlog metadata contains only request identity/category; free-text reasons remain in the private request table and require permission-checked historical access before inspection. Revoked evidence permission withholds reasons. Workers and direct gateway table writes cannot approve or mutate requests.

The private history control includes a reviewer request form with an exact-argument retry after an ambiguous response, and explicit inspection of retained reasons for pending or resolved causes. Navigation/unmount cannot deliver a stale completion into another workspace. Requests are retained after reopening; no persistent browser cache is introduced. Completion still uses 006's all-pending cause explanations, authority checks and atomic resolution; requesting reconsideration is not completing it.

New native and frontend/transport tests require exact-head hosted verification. This closes the explicit human-request entry path only. A full assessment composer, author/reviewer controls, evaluated F3 worker generation and recovery, authoritative method changes, historical commit visibility and rendered visual verification remain incomplete. Owner-gated scoring methods/F2 and production configuration are unchanged. No live deployment, source retrieval or publication is introduced.

The first 007 native run at ed66b7540697867aa96bdcbd7338d8724d59793f passed 46/47 cases; the expected stale-cause rejection closed an ON_ERROR_STOP client, and the test incorrectly reused that closed client. Recovery now opens a fresh client before submitting the updated exact cause set. The failed run remains preserved. Additional regressions reject whitespace-only SQL reasons and duplicate form submissions within the same render. The prior d25b915 browser/World View run 34794093841 has now completed successfully.

## Retained-evidence assessment composer (008 continuation)

The optional private hypothesis client now exposes explicit authoring reads and a complete argument-entry form. Assigned reviewers can define competing explanations, record their relationship and comparison, inspect bounded retained passages, state each passage's documented scope, link inferential arguments/counterevidence, explain limitations, record change tests and explicitly address pending reassessment causes. Initial saves and later completions use the existing immutable acceptance path. A saved entry remains private/unreviewed and does not mark the workspace reviewed.

Authoring context is read-only and bound to the current retained workspace observation. It contains no prior assessment rationale or source passages. It derives field lengths only after the existing six retention/analysis/excerpt × rights/privacy checks; blocked materials have no field data. A passage read repeats those checks, obeys a technical 2,000-code-point response bound, and binds material/version/observation, native UTF-8 hash and source clocks. The browser verifies the returned passage hash before display. This transport bound is neither source admission nor a semantic sample requirement. Source permission and current context are rechecked at final acceptance.

The form records human argument entry (human-argument-entry-v1), not model execution or an approved estimation method. All five dimensions remain separate and not_estimated until an owner-approved method is configured. The three previously specified comparison states are explicit choices; no default 50%, invented qualitative vocabulary, numerical score, self-attested independent origin or publication approval is created. A favored explanation requires a supporting argument. Assumption/gap/change-test lines are trimmed and blank lines removed when freezing a submission; retained passage bytes are never normalized.

Save freezes the exact request, arguments and evidence hashes. Ambiguous results hide the editing content and permit an identical retry; returned receipts must match immutable arguments, with server-assigned identity/completion time. Permission denial clears authoring access. Closing, logout, scope changes and late read/save responses cannot restore prior private text. Opening the form performs no reconciliation, assessment, review approval or source fetch.

New native tests exercise context and passage reads, independent permission fencing, revoked/old scopes, technical span bounds, and actual frontend-generated submissions through both initial and reassessment PostgreSQL acceptance. Frontend/transport tests cover exact Unicode hashes, separate unestimated dimensions, source/identity substitution, missing cause explanations, frozen saves and late responses. Their exact-head CI results remain required at commit creation.

Remaining work is not redefined around manual entry: evaluated F3 generation/computation and durable failure/recovery integration, author/reviewer review controls, authoritative method/dependency change signals, approved estimation methods and semantic evaluation, trustworthy historical commit visibility, production-shaped Auth transport, rendered visual verification and complete end-to-end qualification remain open. No new real-material permissions or model disclosures are authorized by this composer.

At 4d91cff8bc84dc56a6028da4e6208996c98f9637, native hypothesis verification passed 57/57, including frontend-generated payloads accepted through actual PostgreSQL initial and reassessment transactions. The continuation tightens context response validation for string-preserved large positions and blocked field metadata. Exact-head full hosted checks remain required. The prior efd70eaa browser/World View run 34795104997 also completed successfully; that is inherited browser coverage, not visual qualification of this new composer.


## Isolated hypothesis generation worker continuation

009_generation_worker.sql and worker.mjs add a separate hypothesis data plane, sharing the existing workload broker/session, evaluated implementation/source capability and encrypted remote journal. No method, runtime, credential, permission, provider, worker invocation or production route is installed by default. A comparison claim route cannot read the new hypothesis tables.

A verified reviewer requests a bounded hypothesis definition/span specification. The database captures current retained context, source-derived spans, native JSONB byte hash, complete operation permission closure, pending cause identities and permission-checked private human concern details. Method revision and mapping/key revision are pinned; source text is never accepted from that request. Synthetic methods require synthetic permission evidence for every retained input; their computation tests do not qualify semantics. Numerical or qualitative estimation remains disabled in this worker path pending a separately approved method.

Claim and final acceptance require current broker session, exact runtime/RPC, evaluated implementation/source, method revision, reviewer membership, retained context and matching permission revisions. Output is independently constrained in the database to the captured question/hypotheses/spans/clocks/method, private and unreviewed state, separate missing estimates, explicit argument links and the existing inferred-assessment contract. Completion calls existing atomic initial/reassessment acceptance, then inserts output, acknowledgement and exact retry receipt in the same transaction.

The execute-only worker has no table, method approval, reviewer impersonation, broker, broad service_role or publication grants. NOLOGIN ownership and FORCE RLS apply. Method revocation uses identity then publication fences and cannot reactivate an old revision. Permission changes protect the complete generation closure in saved history and append reassessment causes, including undisplayed inputs.

Queue selection rotates investigations within an authorized runtime, retaining blocked candidates and continuing to later eligible work. This is a finite eligible-pending selection guarantee, not a global latency or starvation guarantee under unbounded arrivals, locked transactions, unavailable methods or revoked authority. Processing work is never automatically recycled or force-cancelled. An expired/lost lease remains stranded for explicit reconciliation.

The worker journals exact requests before RPC using a separate hypothesis protocol namespace. A fresh process retries with current authority. Lost claim responses cannot reconstruct lease secrets. Disposable native tests exercise separate no-network, read-only, unprivileged containers and AES-GCM runtime-scoped remote PostgreSQL journal storage; test host custody is distinct from a production custody decision. No source material retrieval is part of these tests.

Verification status: new implementation awaiting first exact-candidate hosted regression run. Do not interpret this section as a PASS manifest. Prior failed/inactive CC attempts and frozen evidence remain unchanged; CC stays closed 3/3 with null activation.

Remaining work is still finite and explicit: connect private generation request/backlog controls to the trusted Auth adapter; explicit generation recovery/reconciliation authoring; authoritative method-change reconsideration signals (revocation currently denies new acceptance and marks generated history stale); complete rendered human review/version flow; production-shaped Auth/custody verification; qualified historical commit visibility; owner-approved assessment methods/F2. Synthetic method execution is not an implemented qualified semantic assessment engine or the coherent independent-review boundary.
