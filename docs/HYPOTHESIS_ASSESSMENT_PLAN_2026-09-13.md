...
| H3 | Private immutable PostgreSQL version store and exact-content retry; CAS predecessor, concurrent completion ordering, current authority, request receipt, immutable evidence/method references, private grants/RLS and durable audit. | Initial isolated PostgreSQL store and trusted-gateway adapter implemented, with append-only rows, exact retry, predecessor conflict, membership fences and server completion time. PGlite durable close/reopen tests added. Native multi-session ordering and disposable-client process loss passed 13 tests at 2fbe9ac51bf34d223d631a18d5f6b1ad41e4110c. Full parent-assessment binding and production-shaped transport remain open; no service_role enters worker scope. |
| H4 | Evidence reader binds exact retained input versions/spans and permission-backed operations, independence/origin relationships, assumptions and inference edges. Reject unsupported source versions and worker self-attestation. | Retained-observation metadata/span adapters and gateway preparation implemented in isolation. Exact native digests, operation/domain scopes and acquisition clocks are checked. Real-material authoritative reader binding remains unavailable; synthetic positives do not qualify it. Atomic binding with revision acceptance is implemented in 003; hosted verification pending for that continuation. |
| H5 | Append-only invalidation/reassessment jobs for new evidence, correction, withdrawal, contradiction, shared-origin discovery and methodology changes. Atomic current-generation acceptance; pending/failed/retry/restart with exact receipts and no force cancellation. | Source/current-workspace acceptance fencing and exact receipt recovery implemented in 003. 005 adds durable pending-cause capture, scoped reconciliation and restart evidence. Generation handoff, F3 computation/acknowledgement and method/contradiction/shared-origin events remain open. |
| H6 | Complete history endpoint and authenticated workspace client/controller: as-known-then uses trusted recorded completion/observation visibility, reconstructed-now uses a newly saved assessment for the same subject period. No historical evidence substitution or client clock authority. | Pure retained-history selector implemented; isolated authenticated handler, permission-checked retained-history endpoint, scoped client and saved-revision/backlog controls implemented. Production Auth wiring and visibility/commit semantics remain unimplemented. |
| H7 | End-to-end private author/reviewer workflow, compare revisions, changed/unchanged/less-certain outcomes, source correction regression, access revocation, actual restart and native concurrency evidence; remote rendered visual inspection. | Saved-revision selection, scoped refresh, pending-cause display and explicit missed-change reconciliation controls are implemented with interaction/access-race tests. Full authoring/reviewer completion and rendered visual verification remain open. |
...
Preparation returns **prepared_requires_atomic_acceptance**, never a committed assessment, eligibility approval or public-release authorization. H5 still must couple current source/method changes, acceptance and durable reassessment receipts atomically; 003 revokes gateway execution of the old unbound 001 append primitive; bound acceptance is the only gateway append after installing that isolated script. Authenticated frontend delivery and committed-observation history remain H6–H7 work.


## Atomic bound acceptance continuation

`003_bound_acceptance.sql` couples the assessment and a non-content binding receipt in one transaction. It rechecks exact observation/version/question identity, retained span digest, source clocks, current membership and six permission scopes under the existing permission/revocation fence. Source change inserts and workspace-head mutations acquire that same fence exclusively before mutation. New acceptance refuses a changed watched source or workspace head; unrelated source changes do not invalidate the question. This is isolated qualification SQL, never a live migration.
...
These tests exercise the actual retained workspace pipeline and synthetic permission fixtures. They do not yet prove native multi-session race ordering, process termination, a durable reassessment worker, method-authority invalidation, authenticated frontend delivery or historical commit visibility. The worker, method authority, transport and commit-visibility items remain finite H5–H7 requirements; the later native continuation supplies multi-session ordering and disposable-client process-loss evidence. No real publisher permissions, CC material, numerical methodology, production principal or release authority is added.
...
Coverage includes concurrent identical requests, competing predecessors, both source-correction and permission-revocation orderings, current membership, rollback, pre-commit process loss, post-commit receipt recovery, fresh-authority retry and narrow grants. The first successful native qualification is run 34790151685 at 2fbe9ac51bf34d223d631a18d5f6b1ad41e4110c (13/13); subsequent candidates require their own run. it does not establish the still-missing F3 hypothesis reassessment worker or production Auth transport.


## Permission-checked history and isolated transport

`004_bound_history.sql` replaces the gateway's unrestricted retained-history primitive. Every returned revision rechecks current membership and its exact accepted permission/span bindings. Revoked/missing permission or a replaced admission/permission revision withholds all saved assessment text and returns an inspectable non-content reason. New permission evidence cannot automatically restore old display. A still-permitted but source-stale record is explicitly historical with reassessment_pending=true; it is never represented as a newly completed conclusion.

The isolated `handler.mjs` derives user identity exclusively from an injected verified Auth reader, binds source namespace to trusted configuration, rejects caller identities/publication/review promotion, constrains origins and request bytes, and sanitizes failures. It is exercised against the actual isolated database store and with adversarial transport tests. No index.ts, live endpoint, service credential or deployment is added. Its history response explicitly states retained_versions_only and historical_commit_visibility_qualified=false: as-known-then completion/commit semantics remain open.

Native testing exposed the SECURITY INVOKER change_subjects view's underlying privilege requirements. 003 now grants only identity/change-position columns and owner-only SELECT policies on the three underlying tables, plus the narrow REFERENCES privileges needed by receipt foreign keys. It does not grant source payload reads or gateway table access. Earlier failed CI remains historical evidence; success must be taken from a later exact candidate.


Final-acceptance defense in depth now also rejects a favored hypothesis with no linked supporting argument; a reports_allegation edge cannot bypass that check by skipping the JavaScript preparation helper. This is a structural argument requirement, not a substantive finding or approved scoring method. The authenticated append handler accepts only unreviewed records; a caller cannot assign reviewed status.


## Durable reassessment cause ledger

`005_reassessment_causes.sql` links the current accepted hypothesis revision to exact existing retained source-change positions, workspace-head changes and per-operation permission/admission changes. Triggers reuse the existing source/permission fence. A current question-level lock serializes reconciliation with acceptance; shared fence acquisition avoids introducing a lock upgrade into ordinary reconciliation. Ledger inserts commit or roll back with the source mutation, and all rows are append-only. No source body or free-form permission evidence is copied into this ledger.

Missed notifications are recovered by anti-membership against the saved observation's exact input positions and watched source identities, never a maximum sequence/time watermark. Reconciliation is scoped to an authorized investigation and returns exact retained cause IDs. Insertion is idempotent. Permission head changes consider both old and new scopes, and reconciliation also detects current denials/revision changes without an incoming notification. Current permissions still gate content acceptance/history independently of whether a ledger notification exists.

Backlog status remains pending_explicit_reconciliation and completed_reassessment=false. A source-change notification does not classify the substantive correction, infer a new conclusion, invent a score, or clear itself because a process restarted. Original revisions and failed attempts remain retained. This is the durable input side of H5; generation handoff, evaluated F3 computation, atomic resolution acknowledgements, and explicit method/contradiction/shared-origin signals are still required. No schedule, worker invocation or automatic approval is added.


## Private history and pending-change controls

The scoped hypothesis client now reaches history, backlog and explicit reconcile operations through the isolated authenticated handler and durable store. A database-backed test exercises that full client/handler/store path with synthetic Auth and existing retained workspace fixtures. This is not verification of a live external Auth issuer or deployment.

`HypothesisAssessmentHistory.jsx` adds saved-revision selection and pending-cause inspection to `PrivateInvestigationWorkspace` when an explicitly configured hypothesis client is supplied. The default remains unconfigured: no live endpoint or production request is introduced. Viewer/reviewer capability is rechecked by the database; the UI offers reconciliation only to a reviewer. Reconciliation is an explicit click, not a schedule or completed reassessment.
