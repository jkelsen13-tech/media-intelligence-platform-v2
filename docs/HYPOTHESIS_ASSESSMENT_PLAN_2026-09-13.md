...
| H6 | Complete history endpoint and authenticated workspace client/controller: as-known-then uses trusted recorded completion/observation visibility, reconstructed-now uses a newly saved assessment for the same subject period. No historical evidence substitution or client clock authority. | Pure retained-history selector implemented; isolated authenticated handler and permission-checked retained-history endpoint implemented. Production Auth wiring, visibility/commit semantics and full controls remain unimplemented. |
