...
Release the migration and frontend as one reviewed batch. Apply the migration to the MIP Supabase project (`qikvmopbtijoebdqosyq`), recheck the validator and grants, then verify the merged Pages deployment. The migration is not applied by the Pages workflow.
