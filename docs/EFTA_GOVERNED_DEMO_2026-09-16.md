...
011_efta_governed_review.sql adds append-only reviewer decisions, admission receipts and private-reader receipts in mip_identity. Approve/correct/reverse actions bind the predecessor and reject conflicting or retired request replays. An approval explicitly binds institutional namespace/id/label/resolution reference, event-time date/evidence basis/uncertainty, privacy/rights references and owner authorization reference. A separately reviewed immutable identity-resolution registry binds each scope origin to an institution. Admission/read requires that exact current resolved record and identical entity payload; missing, revoked, replaced, ambiguous or drifting identities deny access. Resolution changes require predecessor-linked decisions and new source reviews.
