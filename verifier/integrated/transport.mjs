 ...
 return new Promise((resolve,reject)=>{
  ...
  child.on('exit',code=>{
   if(code){
    ...
    const failure=Error((code??'mip_database_denied_'+state+(object?'_'+object:''))+(contexts?'_via_'+contexts:''))
    failure.code=state // Preserve only PostgreSQL SQLSTATE for the trusted handler; no query/payload text.
    reject(failure)
   }
...
const names=new Set(['configuration','issue','journal_runtime','journal_put','journal_get','worker_claim','worker_complete','worker_fail','producer_enqueue','capture_delta','reconciliation','stage_review','release_isolated','release_public','read_isolated_comparison'])
