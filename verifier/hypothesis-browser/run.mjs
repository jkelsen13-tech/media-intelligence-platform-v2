...
for(const [engine,launcher] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  for(const width of [1280,768,390,320]){
   ...
   assert.equal(await disclosure.innerText(),'Why? Evidence, reasoning, alternatives, and assessment history')
   ...
   assert.ok(visible.includes('Stale — reassessment pending'))
