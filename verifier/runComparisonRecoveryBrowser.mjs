...
try {
  ...
  for(const [engine,launcher] of Object.entries({chromium,webkit})) {
    ...
    try {
      ...
      // Follow a real eligible public article; a reload must never retype it as an event.
      await page.setViewportSize({width:1280,height:1000})
      await page.goto(origin+'#/event/2329d38d018940672787a8acddb35a6f/sources',{waitUntil:'networkidle'})
      const openArticle=page.getByRole('button',{name:'Open in News →',exact:true}).first()
      await openArticle.waitFor({timeout:60000})
      await openArticle.click()
      await page.waitForURL(url=>url.hash.includes('/news?') && url.hash.includes('subject_type=article'))
      const articleId=await identity()
      const expanded=page.locator('.news-card-trigger[aria-expanded="true"]')
      await expanded.waitFor()
      const articleTitle=await expanded.locator('h3').innerText()
      const subjectType=page.locator('dt').filter({hasText:/^canonical_subject_type$/}).locator('xpath=following-sibling::dd[1]')
      assert.deepEqual(new Set(await subjectType.allTextContents()),new Set(['article']))
      await page.reload({waitUntil:'networkidle'})
      await expanded.waitFor()
      assert.equal(await identity(),articleId)
      assert.deepEqual(new Set(await subjectType.allTextContents()),new Set(['article']))
      assert.equal(await expanded.locator('h3').innerText(),articleTitle)
      await page.getByRole('tab',{name:'World View',exact:true}).click()
