...
import {runQikJournaledWorker,recoverQikJournaledRequest,resumeQikJournaledWorker} from '../../supabase/functions/source-comparison-generation-candidate/qikWorkerJournal.js'
...
const allowed=new Set(['worker_claim','worker_complete','worker_fail','worker_journal_put','worker_journal_get','worker_journal_pending'])
...
else if(config.mode==='resume')result=await resumeQikJournaledWorker(options)
else throw Error('mode denied')
process.stdout.write(JSON.stringify({result:typeof result==='string'?{state:result}: {state:result.state,...(result.recovered!==undefined?{recovered:result.recovered}:{}),...(result.held!==undefined?{held:result.held}:{})}})+'\n')
process.exit(0)
