...
try{
 ...
 const unauthorized=await nativeFetch('http://127.0.0.1:8000/',{method:'POST'})
 assert.equal(unauthorized.status,403);await unauthorized.text();assert.equal(calls.length,0)
 const started=performance.now()
 const response=await nativeFetch('http://127.0.0.1:8000/',{method:'POST',headers:{authorization:'Bearer synthetic-invoke'}})
 ...
 console.log(JSON.stringify({case:'deno_actual_index_listener_handler',status:'PASS',deno:Deno.version.deno,
  synthetic_request_ms:Math.round((performance.now()-started)*1000)/1000,provider_calls:0}))
}finally{
