...
try {
  ...
  for (const width of [1280,1024,768,390,320]) {
    ...
    await shell.evaluate(el=>{el.scrollTop=0})
    assert.equal(await page.locator('.ws-canonical').getByText('2024-04-08 (date only)',{exact:true}).isVisible(),true,'canonical header preserves date-only precision')
    assert.equal(await inspector.getByText('2024-04-08 (date only)',{exact:true}).isVisible(),true,'inspector agrees with canonical date')
    console.log('MIP_SHARED_DATE_PRECISION_PASS='+JSON.stringify({engine,width,dateOnly:true,headerAndInspectorAgree:true}))
