...
import {runDurableHypothesisWorker,recoverHypothesisRequest} from '../../supabase/qualification/hypothesis-assessments/worker.mjs'
import {syntheticEvaluation} from './syntheticMethod.mjs'
import {createInterface} from 'node:readline'
import {access} from 'node:fs/promises'
const forbidden=['/var/run/docker.sock','/work/verifier/integrated/transport.mjs','/root/.aws/credentials','/work/.git/config','/work/verifier/hypothesis-worker/fixture.mjs','/work/supabase/qualification/mip-cutover-authority/brokerSession.js']
...
lines.on('line',line=>{
 ...
 if(message.start){
  const {session,runtime,key,method}=message
  const rpc=(name,args)=>send('rpc',name,args)
  const journal={putOnce:(k,v)=>send('journal','putOnce',[k,v]),get:k=>send('journal','get',[k])}
  const options={rpc,journal,session,runtime,method:{...method,evaluate:syntheticEvaluation},requestId:()=>randomUUID(),sha256:s=>createHash('sha256').update(s).digest('hex')}
  const work=key?recoverHypothesisRequest({...options,key}):runDurableHypothesisWorker(options)
  work.then(result=>process.send({done:true,state:typeof result==='string'?result:result.state,recovery_key:result?.recovery_key}))
