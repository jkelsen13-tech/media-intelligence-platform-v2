...
export async function savedBoundaryHistoryCases(t,f,prepared){
 ...
 await nativeCase('already-expired session denies; material is withheld; expired JWT denies',async t=>{
  for(const fault of ['material','session','jwt']){
   const x=await context(t);let restore;let expireSql
   if(fault==='material'){
    ...
    expireSql="alter table mip_identity.operation_evidence_versions disable trigger immutable;update mip_identity.operation_evidence_versions set expires_at=clock_timestamp()-interval '1 second' where revision in (select (p->>'revision')::uuid from mip_hypothesis.acceptance_bindings b cross join lateral jsonb_array_elements(b.metadata) e cross join lateral jsonb_array_elements(e->'permissions') p where b.revision_id="+q(x.revisions[0])+");alter table mip_identity.operation_evidence_versions enable trigger immutable"
   }
   if(fault==='session'){
    ...
    expireSql="alter table mip_identity.sessions disable trigger user;update mip_identity.sessions set expires_at=clock_timestamp()-interval '1 second' where session_id="+q(x.b.session())+";alter table mip_identity.sessions enable trigger user"
   }
   if(fault==='jwt'){
    x.request.authorization='Bearer '+x.provider.token({exp:0})
    let delivered=false
   ...
   try{
    // Disposable administrator fault is already expired before admission starts.
    await f.admin(expireSql)
    if(fault==='session'){
