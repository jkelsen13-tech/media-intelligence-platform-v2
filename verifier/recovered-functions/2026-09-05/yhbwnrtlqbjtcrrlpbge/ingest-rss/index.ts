...
// The scheduled path authenticates with a per-project token held only in
// Supabase Vault. The original owner-held INGEST_RSS_RUN_KEY remains valid for
// manual operational runs; neither token is stored in source or config tables.
async function hasSchedulerAuthorization(supabase: any, req: Request): Promise<boolean> {
  const token = req.headers.get('x-ingest-rss-scheduler-token')
  if (!token) return false
  const { data, error } = await supabase.rpc('mip_ingest_rss_schedule_authorized', { p_token: token })
...
async function attachToArc(supabase: any, art: any, arc: any, ctx: AttachContext) {
  // Arc & Collections default-deny gate: selection signals may create a private
  // candidate, but only arc-membership-run plus its release policy may attach.
  // Returning here also prevents unadmitted nodes, sources, edges, and topic
  // rows from being created through this legacy direct-attach call path.
  const { error: stageErr } = await supabase.from('arc_membership_candidates').upsert({
    article_id: art.id,
    arc_id: arc.id,
    generation_method: 'ingest_entity_similarity_candidate_v1',
    generation_evidence: {
      similarity: ctx.similarity,
      shared_entity_ids: ctx.sharedEntityIds,
      shared_entity_names: ctx.sharedEntities,
      prior_rule_version: 'arc_assign@20260724+gte-small(threshold=0.88)',
      staged_at: new Date().toISOString(),
    },
    state: 'pending',
  }, { onConflict: 'article_id,arc_id' })
  if (stageErr) throw stageErr
  return { staged: true }

  /* Legacy public-projection mutations are intentionally disabled. The
     guarded approval path will create a member only after model admission.
...
Deno.serve(async (req: Request) => {
  // Fail closed. Manual operations retain the owner-held run key; the only
  // scheduled path is the Vault-backed token verified by a service-role RPC.
  // Both credentials must be absent or invalid for writes to proceed.
  ...
  const ownerAuthorized = !!expected && req.headers.get('x-ingest-rss-key') === expected
  const schedulerAuthorized = await hasSchedulerAuthorization(supabase, req)
  ...
  try {
    ...
    if (Number.isInteger(payload?.source_index) && Number(payload.source_index) >= 0) {
      requestedSourceIndex = Number(payload.source_index)
    }
  } catch {
    // Empty or non-JSON bodies use the normal clock-based rotation.
  }
  ...
  const FEED_FETCH_TIMEOUT_MS = Number(cfg.feed_fetch_timeout_ms ?? 5000)
  const RSS_FEEDS_PER_RUN = Number(cfg.rss_feeds_per_run ?? 1)
  const RSS_SCHEDULE_SLOT_MINUTES = Number(cfg.rss_schedule_slot_minutes ?? 5)
  const SOURCE_COMPARISON_ARC_SYNC_LIMIT = Number(cfg.source_comparison_arc_sync_limit ?? 500)
  ...
  const PRIOR_POOL_LIMIT = Number(cfg.prior_unattached_pool_limit ?? 12)
  ...
  const { data: sourceRows, error: srcErr } = await supabase
    .from('ingest_sources')
    .select('id, feed_url, enabled, outlets (id, name)')
    .eq('enabled', true)
    .order('id', { ascending: true })
  if (srcErr) throw srcErr
  const enabledSources = sourceRows ?? []
  // Scheduled executions rotate deterministically across feeds. A single feed
  // plus a bounded retry pool keeps each Edge Function execution below its
  // worker budget while the 5-minute scheduler covers every source per cycle.
  const scheduledSourceStart = enabledSources.length > 0
    ? requestedSourceIndex !== null
      ? requestedSourceIndex % enabledSources.length
      : Math.floor(Date.now() / (Math.max(1, RSS_SCHEDULE_SLOT_MINUTES) * 60_000)) % enabledSources.length
    : 0
  const sources = schedulerAuthorized
    ? Array.from({ length: Math.min(Math.max(1, RSS_FEEDS_PER_RUN), enabledSources.length) }, (_, offset) => enabledSources[(scheduledSourceStart + offset) % enabledSources.length])
    : enabledSources
  ...
  const runId = `rss-${new Date().toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}-${crypto.randomUUID().slice(0, 8)}`
  const { error: runStartErr } = await supabase.from('ingestion_runs').insert({
    run_id: runId,
    mode: 'discover',
    state: 'running',
    source_window_start: cutoff.toISOString(),
    source_window_end: new Date().toISOString(),
    algorithm_version: 'ingest-rss-v10-scheduled-telemetry',
    model_id: EMBED_MODEL,
    counters: {},
    notes: schedulerAuthorized
      ? requestedSourceIndex === null ? 'Scheduled pg_cron run.' : `Scheduled recovery slot ${requestedSourceIndex}.`
      : 'Owner-key operational run.',
  })
  ...
  const report: any = {
    ...
    thresholds: {
      ...
      feed_fetch_timeout_ms: FEED_FETCH_TIMEOUT_MS,
      feeds_per_run: RSS_FEEDS_PER_RUN,
      schedule_slot_minutes: RSS_SCHEDULE_SLOT_MINUTES,
      prior_unattached_pool_limit: PRIOR_POOL_LIMIT,
    },
    feeds: [] as any[],
    ingested: 0,
    skippedExisting: 0,
  ...
  for (const src of sources ?? []) {
    // Continue fetching every enabled endpoint after the intake cap is reached:
    // source health is an operational signal independent of admitted volume.
    ...
    const { error: sourceRunErr } = await supabase.from('ingestion_source_runs').upsert({
      run_id: runId,
      ingest_source_id: (src as any).id,
      outlet_name: outlet?.name ?? null,
      feed_url: src.feed_url,
      state: feedReport.error ? 'failed' : 'succeeded',
      fetched_items: feedReport.fetched,
      new_items: feedReport.new,
      error_note: feedReport.error ?? null,
      completed_at: new Date().toISOString(),
    }, { onConflict: 'run_id,ingest_source_id' })
    if (sourceRunErr) throw sourceRunErr
  }
  ...
  // Project only qualifying multi-outlet arc memberships into the canonical
  // event representation. This is deliberately a pending-review write; it
  // never changes Source Comparison scoring or approval state.
  const { data: sourceComparisonSync, error: sourceComparisonSyncErr } = await supabase.rpc(
    'mip_sync_arc_source_comparison_events',
    { p_limit: SOURCE_COMPARISON_ARC_SYNC_LIMIT },
  )
  if (sourceComparisonSyncErr) throw sourceComparisonSyncErr
  report.sourceComparisonEventSync = Array.isArray(sourceComparisonSync)
    ? sourceComparisonSync[0] ?? null
    : sourceComparisonSync ?? null

  const completedAt = new Date().toISOString()
  const { error: runCompleteErr } = await supabase
    .from('ingestion_runs')
    .update({
      state: report.errors.length > 0 ? 'completed_with_errors' : 'completed',
      completed_at: completedAt,
      source_window_end: completedAt,
      counters: report,
      notes: report.errors.length > 0
        ? `Completed with ${report.errors.length} feed error(s); inspect ingestion_source_runs for endpoint evidence.`
        : 'Completed successfully.',
    })
    .eq('run_id', runId)
  if (runCompleteErr) throw runCompleteErr
