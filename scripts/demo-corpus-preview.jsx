...
const allSources = investigations.flatMap((item) => item.sources)
...
function SourceCard({ source, selected, onSelect }) {
  return <button type="button" className={`demo-source-card${selected ? ' selected' : ''}`} onClick={() => onSelect(source)} data-capture-id={source.capture_id}>
    ...
    <span className="demo-source-card__meta"><span>{pretty(source.semantic_kind)}</span><span>Capture {shortId(source.capture_id)}</span></span>
  </button>
}
...
function SourceFeed({ investigation, query, selected, onSelect, onOpenEvidence }) {
  ...
  const sources = investigation.sources.filter((source) => !needle || [source.title, source.outlet, source.statement, source.semantic_kind].some((value) => String(value ?? '').toLowerCase().includes(needle)))
  ...
  return <div className="demo-native-view demo-feed" ref={selectedRef}><div className="demo-view-heading"><div><p className="demo-eyebrow">News</p><h2>Retained source feed</h2><p>{sources.length} of {investigation.sources.length} private records</p></div><PendingBadge>All pending</PendingBadge></div><DemoBoundaryNote compact /><div className="demo-filter-row" aria-label="Discovery filters"><span><MagnifyingGlass size={14} /> Search is local to this retained collection</span><span>Review: Pending</span><span>Geography: Withheld</span></div>{selected && <section className="demo-mobile-selection"><p className="demo-eyebrow">Selected retained source</p><h3>{selected.title}</h3><p>Capture {shortId(selected.capture_id)} · candidate {shortId(selected.candidate_id)}</p><button type="button" className="demo-native-link" onClick={onOpenEvidence}><Fingerprint size={15} /> Open provenance and exact-span identity</button></section>}{sources.length ? <div className="demo-source-grid">{sources.map((source) => <SourceCard key={source.capture_id} source={source} selected={selected?.capture_id === source.capture_id} onSelect={onSelect} />)}</div> : <div className="demo-honest-empty"><h3>No matching retained sources</h3><p>The search remains local and does not query production.</p></div>}</div>
}
...
function graphProjection(investigation) {
  const origins = new Set(), nodes = [], edges = []
  for (const source of investigation.sources) {
    const originKey = source.origin_id ? `private:origin:${source.origin_id}` : 'private:origin:unresolved'
    if (!origins.has(originKey)) { origins.add(originKey); nodes.push({ id: originKey, label: source.origin_id ?? 'Origin unresolved', type: 'source_origin', metadata: { entity_type: 'receipt_origin_label', demo_semantics: 'receipt_origin_label_not_canonical_actor' } }) }
    nodes.push({ id: source.preview_id, label: source.title, type: 'document', capture_id: source.capture_id, metadata: { demo_semantics: 'retained_private_capture' } })
    edges.push({ id: `private:attribution:${source.capture_id}`, source: source.preview_id, target: originKey, type: 'documentary', label: 'retained from declared origin', weight: 'light', claimed_by: 'source_document', publication_allowed: false })
  }
  return { nodes, edges }
}

function NativeGraph({ investigation, onSelect }) {
  const graph = useMemo(() => graphProjection(investigation), [investigation])
  const byId = useMemo(() => new Map(investigation.sources.map((source) => [source.preview_id, source])), [investigation])
  return <div className="demo-native-view demo-graph-view"><div className="demo-view-heading"><div><p className="demo-eyebrow">Graph</p><h2>Evidence custody and attribution</h2><p>Document nodes connect only to receipt-declared origin labels.</p></div></div><DemoBoundaryNote compact /><div className="demo-graph-legend"><span><i className="origin" /> Declared origin label</span><span><i className="capture" /> Retained capture</span><span><GitFork size={14} /> No actor, causal, or event-equivalence edges</span></div><div className="demo-graph-stage"><GraphView nodes={graph.nodes} edges={graph.edges} onSelect={(node) => onSelect(byId.get(node?.id) ?? null)} panelOpen={false} focused /></div></div>
}
...
function NativeDemoApp() {
  ...
  const investigation = investigations.find((item) => item.topic === route.topic) ?? investigations[0]
  ...
  const selectSource = (source) => writeRoute(route.topic, route.surface === 'context' ? 'news' : route.surface, source.capture_id)
  const chooseInvestigation = (topic) => { setQuery(''); setExploreOpen(false); writeRoute(topic, 'context') }
  let content
  ...
  else if (route.surface === 'graph') content = <NativeGraph investigation={investigation} onSelect={(source) => source && selectSource(source)} />
  ...
  return <div className="app ws-app demo-native-app"><InvestigationWorkspace view={view} onChangeView={openView} investigationContext={investigationContext} header={header} selectedChild={selected ? { label: selected.title } : null} hasNativeInspector={false} onChangeInvestigation={() => setExploreOpen(true)} corpusLine="Private corpus — 93 retained — all pending" searchSlot={<WorkspaceSearch exploreOpen={exploreOpen} onOpenExplore={() => setExploreOpen(true)} dialogId="demo-explore" query={query} onQueryChange={(value) => { setQuery(value); if (route.surface !== 'news') writeRoute(route.topic, 'news', selected?.capture_id ?? null) }} />} accountSlot={<WorkspaceAccountButton enabled onClick={() => setAccountOpen(true)} />} infoSlot={<WorkspaceInfoButton onClick={() => setAboutOpen(true)} />} leftNav={<>{DEMO_NAV_ITEMS.map((item) => <WorkspaceNavButton key={item.key} item={item} active={view === item.key} onClick={() => openView(item.key)} />)}</>} inspectorSlot={<ProvenanceInspector source={selected} investigation={investigation} surface={route.surface} onOpenEvidence={() => writeRoute(route.topic, 'evidence', selected?.capture_id ?? null)} />} inspectorSelection={selected} details={<details className="ws-details"><summary>Investigation details &amp; demo boundary</summary><div className="demo-details"><DemoBoundaryNote /><p>Static build-time provider · no Supabase client · no production backend · no admission or publication operation.</p><p>Selected collection: {titleForTopic(route.topic)} · {investigation.sources.length} pending records.</p></div></details>}><main className="app-main">{content}</main></InvestigationWorkspace>
    ...
    {accountOpen && <Modal title="Account" onClose={() => setAccountOpen(false)}><div className="sheet-body"><p><CheckCircle size={18} /> Access is enforced by Vercel Authentication before this artifact loads.</p><p>No MIP production authentication client is included in the demo bundle.</p></div></Modal>}
  </div>
}
