...
export function buildReceiptPropagation(universe, graph) {
  ...
  const sourceReceipts = sources.map((source, index) => {
    ...
    const stages = {
      ...
      publication_time: stage(publication.dateTime ? 'recorded' : 'unavailable', publication.dateTime ? 1 : 0, 'Retained publication clock only; source_date never supplies a missing timestamp. Source update timestamp unavailable: no source update field is supplied; capture or verification time cannot substitute.'),
      ...
      graph_edges_by_type: stage('declared_provenance_only', incidentEdges.length, 'Receipt-declared unverified provenance display edges only; no accepted semantic relationship.'),
    ...
    return { contract: PROPAGATION_CONTRACT, ...roots, publication_allowed: false,
      ...
      publication: { published_at: source.published_at, precision: source.publication_precision, display: publication,
        source_updated_at: null, source_update_state: 'unavailable',
        source_update_reason: 'No source update timestamp is supplied; capture or verification time cannot substitute.',
